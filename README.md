...
| Historical exploit replay (the brief's core validation requirement) | **Partial.** One case (Beanstalk, Apr 2022) verified against Etherscan; its real transaction replays live on a real mainnet fork (Solidity; asserts real token outflow from the Beanstalk diamond). Scoring its real call trace with the actual Rust detection engine (`crates/replay-harness`, using two independently-verified real function selectors) is written but has **not run live** — it needs an RPC plan with trace methods, which free tiers don't include. Three more signature-diversity cases (flash-loan, oracle-manipulation, reentrancy) are scaffolded but explicitly left unverified rather than filled with unconfirmed data — see `PLAN.md` |
