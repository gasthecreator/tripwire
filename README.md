...
| Historical exploit replay (the brief's core validation requirement) | **Partial: two of four real cases, both run live.** **Beanstalk** (Apr 2022) and **Euler Finance** (Mar 2023): transaction hashes verified (Euler was found on-chain first, via Aave `FlashLoan` events, then confirmed on Etherscan); each replays on a real mainnet fork in Solidity (Beanstalk: real token outflow from the diamond; Euler: DAI balance 8.9M → 0), and its real call trace (`cast run` re-executes the transaction locally, so no paid trace API is needed; Euler also gets a real fund-flow baseline from archive balances and receipt logs) is scored by the actual Rust detection engine. Signatures tuned to each incident reach 95.0 against an 80.0 threshold — that shows the pipeline works on real traces, **not** that generic signatures would have caught them: the shipped generic set scores 65.0 on Beanstalk (a legitimate nested flash-swap callback re-entry; below threshold) and 60.0 on Euler (the balance drain alone; below threshold). Replaying these found and fixed two real detector bugs that synthetic tests missed: a reentrancy false positive (read-only calls and already-returned calls counted as re-entry) and evidence double-counting (one outflow summed three times, which would have paused a protocol on any large legitimate withdrawal) — see `PLAN.md`. Two more cases (oracle manipulation, reentrancy) are scaffolded but explicitly left unverified rather than filled with unconfirmed data |
