...
| Historical exploit replay (the brief's core validation requirement) | **Partial.** Two cases (Beanstalk, Apr 2022; Euler Finance, Mar 2023) verified against Etherscan (Euler found on-chain first); its real transaction replays live on a real mainnet fork (Solidity; asserts real token outflow from the Beanstalk diamond). Its real call trace (349 frames, produced by `cast run` re-executing the transaction locally, so no paid trace API is needed) is scored by the actual Rust detection engine (`crates/replay-harness`): a signature built from two independently-verified real selectors reaches confidence 95.0 against an 80.0 threshold. That signature was tuned to this incident, so it shows the pipeline works on a real trace, not that a generic signature would have caught it — the shipped generic signatures score 65.0 on the same trace (below threshold; that comes from a legitimate nested flash-swap callback re-entry, after a real false positive in the reentrancy condition — read-only calls and already-returned calls counted as re-entry — was found by this trace and fixed, see `PLAN.md`). |
